...
vows.describe('rating for').addBatch(
  'one match with':
    ...
    'exact draw should give 4 points': (match) ->
      assert.equal match.score([0,0], [0,0]), 4
      assert.equal match.score([3,3], [3,3]), 4
    'exact home win shouldgive 4 points': (match) ->
      assert.equal match.score([1,0], [1,0]), 4
    'exact away win should give 4 points': (match) ->
      assert.equal match.score([0,2], [0,2]), 4
  ...
  'multipe matchs with':
    ...
    'two exact matches should give 8 points': (group) ->
      estimated = 1: [0,0], 2: [2,0]
      real      = 1: [0,0], 2: [2,0]
      assert.equal group.score(estimated, real), 8
    'no correct results should give 0 points': (group) ->
      estimated = 1: [0,0], 2: [2,0]
      real      = 1: [1,0], 2: [0,0]
      assert.equal group.score(estimated, real), 0
    'two tendencies and one exact draw 8 points': (group) ->
